...
クエリの仕様:
- クエリはキーワード検索（完全一致）と意味ベクトル検索（類似度）の両方に使われます。
- 固有名詞やキーワードを含めるとキーワード検索でヒットしやすくなります。
- 自然な質問文にすると意味検索でヒットしやすくなります。
- 1回の検索で十分な情報が得られない場合は、クエリを言い換えて複数回検索し、結果を統合して推論してください。
